...
const getStatusStyle = (status) => {
  ...
  return "bg-slate-400 text-white text-[10px]";
};
...
// ── Table component with memoization
const LogsTable = React.memo(
  ({ data, currentFilter, onFilterChange, onDeleteRequest }) => {
    ...
    const mappedData = useMemo(() => {
      return data.map((log) => {
        ...
        // Construct user name if missing
        if (log.user && !userObj.name) {
          userObj.name = `${log.user.firstName || ""} ${log.user.lastName || ""}`.trim() || log.user.email || "Unknown";
        }
    ...
    const displayed = useMemo(() => {
      return mappedData.filter((row) => {
        const matchStatus =
          currentFilter === "all" || row.status?.toLowerCase() === currentFilter;
        ...
        return matchStatus && matchAction && matchRole && matchSearch;
      });
    }, [mappedData, currentFilter, actionFilter, roleFilter, search]);
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="text-slate-400 text-xs uppercase tracking-wider border-b border-slate-100 bg-slate-50/50">
                ...
                <th className="px-6 py-3 w-12 text-right">
              ...
              {displayed.length > 0 ? (
                displayed.map((row) => (
                  ...
                  >
                        ...
                        <div className="flex flex-col min-w-0">
                          ...
                          <span
                            className={`text-xs truncate leading-snug ${
                              ROLE_STYLES[row.user.role] || "text-slate-400"
                            }`}
                          >
                            {row.user.role}
                          </span>
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide ${getStatusStyle(
                          row.status
                        )}`}
                      >
                        {row.status || "PENDING"}
                    ...
                    <td className="px-6 py-4 text-right">
                      <button
                        ...
                        className="text-slate-400 hover:text-red-500 transition-colors p-1.5 rounded-lg hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2"
                        title="Delete this log entry"
                        aria-label={`Delete log for ${row.user.name}`}
                      >
                        <Trash2 size={15} />
                      </button>
                    </td>
